...
int main(int argc, char *argv[]) {
	...
	/* Create first thread for I2C communication and datapool updating */
	pthread_attr_init(&tattr);
	pthread_attr_setdetachstate(&tattr, PTHREAD_CREATE_DETACHED);
	ret = pthread_create(&tid, &tattr, receive_datapool, NULL);
	if(ret != SUCCESS) {
		printf("error: could not start thread for I2C communication\n");
	}
